...
DA Builder is a data availability blob aggregation serveice that takes type 3 blob transactions from L2 rollups and aggreagtes them with other roll up transactions and posts them on behalf of rollup proposers. To do this we need to take payment for gas and be able to sign transactions using a hot wallet on behalf of the proposer. To keep gas costs down we record payments off chain and charge the proposer on a scheduled basis.
