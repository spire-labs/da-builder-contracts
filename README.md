...
### IProposer
This is the expected interface for accounts to delegate to using 7002, we provide an example implementation for a 7702 proposer account for users to use, however if someone wants to use their own implementation they can, the only requirement is it supports the `IProposer` interface.